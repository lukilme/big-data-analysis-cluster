...
SELECT
    RANK() OVER (ORDER BY count_login DESC) AS rank,
    host,
    count_login
FROM (
    SELECT
        host,
        COUNT(*) AS count_login
    FROM dwd_login
    GROUP BY host
) AS user_counts
ORDER BY count_login DESC
LIMIT 10;
...
SELECT
    time_msg,
    ROUND(COUNT(*) * 1.0 / COUNT(DISTINCT host), 2) AS logins_per_device
FROM dwd_login
GROUP BY time_msg
ORDER BY time_msg;
...
SELECT
  COUNT(IF(login_count BETWEEN 1 AND 5, 1, NULL)) AS logins_1_to_5,
  COUNT(IF(login_count BETWEEN 6 AND 10, 1, NULL)) AS logins_6_to_10,
  COUNT(IF(login_count > 10, 1, NULL)) AS logins_above_10
FROM (
  SELECT host, COUNT(*) AS login_count
  FROM dwd_login
  GROUP BY host
) aux;
...
WITH hours_count AS(
    SELECT 
        HOUR(FROM_UNIXTIME(time_msg)) AS hour_time,
        COUNT(*)
    FROM dwd_login
    GROUP BY HOUR(FROM_UNIXTIME(time_msg));
)
